...
impl TryFrom<&str> for Rgb {
    ...
    fn try_from(hex: &str) -> Result<Self, Self::Error> {
        ...
        // actual processing
        let (r, g, b) = (
            u8::from_str_radix(&hex[1..3], 16).map_err(RgbError::ParseIntError)?,
            u8::from_str_radix(&hex[3..5], 16).map_err(RgbError::ParseIntError)?,
            u8::from_str_radix(&hex[5..7], 16).map_err(RgbError::ParseIntError)?,
        );
        Ok(Rgb(r, g, b))
    }
}

// can just use the `#[from]` attribute macro on the error type to skip this
// // to use the ? operator, we need to implement From for the error type
// impl From<std::num::ParseIntError> for RgbError {
//     fn from(e: std::num::ParseIntError) -> Self {
//         RgbError::ParseIntError(e)
//     }
// }
